...
with col1:
    with open(r"c:\local git reposiroty\Camp-60-Days-of-Code\day_23\images\photo.png", "rb") as f:
        image = f.read()
    st.image(image)
...
df = pandas.read_csv(r"c:\local git reposiroty\Camp-60-Days-of-Code\day_23\data.csv", sep=";")

with col3:
    for index, row in df[:10].iterrows():
        st.header(row["title"])
        st.write(row["description"])
        with open(rf"c:\local git reposiroty\Camp-60-Days-of-Code\day_23\images\{row['image']}", "rb") as f:
            image = f.read()
        st.image(image)
